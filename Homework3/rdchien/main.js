...
d3.csv("ds_salaries.csv", d => {
    ...
    svg.selectAll("mybar")
        ...
        .attr("y", function (d) { return y(0); }) // Start bars at the bottom
        .attr("height", 0) // Start with height 0
        .attr("width", x.bandwidth())
        .attr("fill", function (d) { return colorScale(d.key); })
        .transition() // Apply transition
        .duration(800) // Transition duration in milliseconds
        .attr("y", function (d) { return y(d.value); }) // End position of the bars
        .attr("height", function (d) { return height - y(d.value); }) // End height of the bars
        .delay(function (d, i) { return i * 100; }); // Add a slight delay between each bar's animation
